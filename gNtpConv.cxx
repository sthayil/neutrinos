...
void ConvertToGST(void)
{
  ...
  double brXSec        = 0;      // Event cross-section ##STEF
  double brDiffXSec    = 0;      // Differential cross-section ##STEF
  ...
  double brChargel     = 0;      // Final state primary lepton charge STEF
  ...
  double brChargef  [kNPmax];       // Charge of k^th final state particle in hadronic system #STEF
  ...
  s_tree->Branch("xsec",          &brXSec,          "xsec/D"	    );//STEF
  s_tree->Branch("diffxsec",      &brDiffXSec,      "diffxsec/D"    );//STEF
  ...
  s_tree->Branch("chargel",	  &brChargel,	    "chargel/D"	    );//STEF
  ...
  s_tree->Branch("chargef",       brChargef,	    "chargef[nf]/D" );//STEF
  ...
  for(Long64_t iev = 0; iev < nmax; iev++) {
    ...
    // Get event weight and xsecs
    double weight = event.Weight();
    double xsec = event.XSec()/units::cm2 ;//STEF
    double diffxsec = event.DiffXSec()/units::cm2 ;//STEF
...
//    }
    ...
    while( (p = (GHepParticle *) piter.Next()) && study_hadsyst)
    {
      ...
      //if(event.Particle(ip)->FirstMother()==0) continue; //STEF get rid of this line?
      if(pdg::IsPseudoParticle(p->Pdg())) continue;
      int pdgc = p->Pdg();
      int ist  = p->Status();
      if(ist==kIStStableFinalState) {
	 // STEF commented out this block-- I want the es and gammas from pions too
	...
	final_had_syst.push_back(ip);
      ...
      // STEF got rid of this block too, I don't want these pi0s
    ...
    LOG("gntpc", pINFO) 
     ...
     << ", N(gamma,e-,e+):" << brNiEM
     << "filled xsec: " <<brXSec/units::cm2
     << ", N(etc):"         << brNiOther << "LALALLALALLALALALLALALLALALALLALALAL \n"; //stef
